...
$registComp("105", {
	...
	//世界杯组件框架模板	
	cupTemp : [
	'<div class="worldcup_widget">',
	    '<div class="article_head">',
	        '<h2>我的世界杯，盛夏激情记录</h2>#{activityLinks}',
	    ...
	    '<div class="article_main">',
	        '<h3 class="widget_title01" style="display:#{titleDisplay}">',
	            //'<span class="SG_more"><a href="#">更多</a>&gt;&gt;</span>',
	        '</h3>',
	        '<div class="scoreList" style="display:#{scoreDisplay}">#{gameScore}<div class="clearit"></div></div>',	//赛事结果
	        '<div class="myWorldcup_bottom" style="display:#{articleDisplay}">',
	            '<h3 class="widget_title02"></h3>',
	            '<div class="blogRecordList">',
	                '<ul>#{gameArticle}</ul>',		//赛事博文。
	            '</div>',
	...
	'<div class="worldcup_widget">',
	    ...
	    '</div>',
	    '<div class="article_main">',
	        '<h3 style="display:#{titleDisplay}" class="widget_title01">',
	        	//'<span class="SG_more"><a href="#">更多</a>&gt;&gt;</span>',
	        '</h3>',
	        '<div class="scoreList" style="display:#{scoreDisplay}">',
	            '<ul>#{gameScore}</ul>',			//
	...
	'<li>',
	    '<span class="title">',
	        '<a href="#{link}" target="_blank">#{title}</a>',		//文章名字、文章链接。
	    '</span>',
	    '<span class="author"><a href="#{authorBlog}" target="_blank">#{author}</a></span>',	//作者
	...
	'<div class="article_head_nav">',
	    ...
	    '<a href="#{cupUserLinks}" target="_blank">我的世界杯记录</a>',
	'</div>'].join(""),
	...
	load : function(){
		...
		(function(){
			if(_this.isGameInfoOk && _this.isArticleOk){
				//trace("is sniking");
				switch(_this.size){
					case 210: _this.render("210"); break;
					case 510: _this.render("510"); break;
					case 730: _this.render("730"); break;
				}
			}else{
				//trace("waiting");
...
// "1":{"teams":["1","2"],"time":"2010-04-20 13:00:00","score":"1:2","group":"A","chatid":3490001},
		...
		switch(mode){
			...
			case "510":
				...
				if($isAdmin) tempLinks = this.masterLinks;
				else tempLinks = this.guestLinks;
				break;
			case "730":{
				tempBox = this.cupTemp;
				tempScore = this.cupCompeteTemp;
