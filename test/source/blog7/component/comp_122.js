       ...
       // data = data.data;
        ...
        for (var m = 0, mlen = data.length; m < mlen; m++) {
           ...
           // var url = 'http://control.blog.sina.com.cn/admin/article/article_add.php?film_id=' + data[m]["filmid"] + '&type=' + type;
            ...
            html += ['<li class="cur">', '<a href="' + url + '" target="_blank" title="'+filmname+'"><img alt="' + filmname + '" src="' + imgurl + '"></a>', 
            ...
            html += ['</ul>', 
					dot, 
					'<div class="clearit"></div>', '</li>'].join("");
        };
        
        var innerHTML = ['<div class="hotmovies">', '<ul>', html, '</ul>', '</div><div class="clearit"></div>'].join("");
