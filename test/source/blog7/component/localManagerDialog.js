...
LocalManagerDialog.prototype= {
	...
	,"interfaceLoadList": new Interface('http://control.blog.sina.com.cn/riaapi/photoblog/get_blog_list_by_page_class.php', 'jsload')
	,"interfaceSave": new Interface('http://control.blog.sina.com.cn/riaapi/photoblog/set_blogs_places.php', 'ijax')
	,"saveData": {}
	,"pageSize": 10
	,"dialog": null
		...
		_this.interfaceLoadList.request({
			...
			,onSuccess: function(oData){
				...
				for (var n = 0, m = oData.length; n < m; n++) {
					var _d = oData[n];
					var htmlSelect = _this.selectTpl.format(_d.blog_id, _d.blog_id, _d.blog_id, 
					"onchange = \"localManagerDialog.changeNation('" + _d.blog_id + "', this.value);\"",
					"onchange = \"localManagerDialog.changeProvince('" + _d.blog_id + "', this.value);\"",
					"onchange = \"localManagerDialog.changeCity('" + _d.blog_id +  "', this.value);\""
					);
					...
					var str_hover = 'onmouseover="this.className=\'mapLayer_cell SG_j_linedot1 mapLayer_hover\'" ' +
								'onmouseout="this.className=\'mapLayer_cell SG_j_linedot1\'" ';
	...
	,"changeProvince": function(blog_id, val, noSave){
		...
		if ($E('nation_' + blog_id).value == 0) { //国内
			...
			if ($E('nation_' + blog_id).value == 1) { //国外
				if (!noSave) {
					if ($E('province_' + blog_id).value == 0) {
						delete this.saveData[blog_id];
					}
	...
	,"tpl": '<div style="width: 750px;" class="CP_layercon6">' +
				'<div class="mapLayer">' +
					'<div class="mapLayer_top SG_j_line1">' +
						'<div class="sort">' +
						'<span>分类：</span>' +
						'<select id="comp_10007_selectCate"><option>加载中…</option></select>' +
						'</div>' +
						'<span class="tip">*为必填项</span>' +
						'<div class="clearit"></div>' +
					'</div>' +
					'<div class="mapLayer_main">' +
						'<div class="mapLayer_list" id="comp_10007_photoBlogList" style="height:384px">' +
						'</div>' +
						'<div id="comp_10007_tips" style="text-align:right;color:#C00;padding-top:10px;overflow:hidden;height:0;display:none">' +
						'</div>' +
						'<div id="comp_10007_set_page" class="SG_page">' +
						'</div>' +
						'<div class="mapLayer_btn">' +
							'<a class="SG_aBtn SG_aBtnB" href="#" id="comp_10007_saveBtn" onclick="return false"><cite>保存设置</cite></a>' +
							'<a class="SG_aBtn SG_aBtnB" href="#" id="comp_10007_cancelBtn" onclick="return false"><cite>关闭窗口</cite></a>' +
						'</div>' +
					'</div>' +
				'</div>' +
			'</div>'
	,"itemTpl": '<div class="mapLayer_cell SG_j_linedot1" {7}>' +
					'<p class="mapLayer_nm SG_dot">{5}<a href="{4}" target="_blank">{0}</a>{6}</p>' +
					'<p class="mapLayer_opt" id="comp_10007_set_{1}" blog_id="{2}">{3}</p>' +
					'<div class="clearit"></div>'+
				'</div>'
	,"selectTpl":'<em>*</em> ' +
				'<select class="m_w1" id="nation_{0}" {3}><option value="-1">请选择</option><option value="0">国内</option><option value="1">国外</option></select> ' +
				'<em>*</em> ' +
				'<select class="m_w2" id="province_{1}" {4}><option value="0">请选择</option></select> ' +
				'<select class="m_w3" id="city_{2}" {5}><option value="0">请选择</option></select>'
	,"chinaMap": {
		1: {
			'pro': '北京',
			'py': 'beijing',
