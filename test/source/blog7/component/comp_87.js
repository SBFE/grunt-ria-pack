...
$registComp(87, {
	...
	"htmlTemplate"	: ['<li class="videoCell">'
						, '<div class="videoReview">'
							...
							, '<img src="#{videoImage}" width="' + (this.size == 730 ? 120 : 80 )
								 +'" height="' + (this.size == 730 ? 90 : 60) + '" alt="#{nameAll}"'
	...
	,"load"		: function () {
		...
		i_getLastestVideo.request({
			GET		: {
				...
				,"channelId": ""
				,"page"		: 1
				,"style"	: currentStyle[this.size]
				,"varname"	: "requestId_vl_" + scope.$uid
			}
...
//					: '</div>');
			result += '<div class="zVideoMore"><span class="SG_more SG_floatR"><a target="_blank" href="'
						+ 'http://you.video.sina.com.cn/vlist/' + scope.$uid + '.html">更多</a>&gt;&gt;</span></div></div>';
