...
slideBox.prototype = {
    ...
    initialize : function(boxBody, slideBody, previousBtn, nextBtn, cfg){
        ...
        this.currentPage = 1;
        this.cfg = cfg;
        var __bind2 = Core.Function.bind2,
            //默认配置
            defaultCfg = {
                ...
                onTweenEnd : function(){} // 每次缓动后的回调函数
            };
        // 绑定上一页和下一页按钮
    ...
    getCurrentSlideLength : function(){
        var currentSlideLength=0,
            slideBodyStyle = this.slideBody.style;
    ...
    getSlideLegnth : function(){
        ...
        var slideBody = this.slideBody,
            children = slideBody.children,
            childCounts = children.length;
        if (childCounts < 1){
            return 0;
        }
        var el = children[0],
            computeStyle = computeStyle || (window.getComputedStyle ? window.getComputedStyle(el, '') : el.currentStyle);
    ...
    previousPage : function(){
        ...
        var _boxBody = this.boxBody,
            _slideBody = this.slideBody,
            computeStyle = window.getComputedStyle ? window.getComputedStyle(_slideBody, '') : _slideBody.currentStyle,
            beginValue = this.getCurrentSlideLength(computeStyle), endValue,
            slideLength = this.getSlideLegnth();
    ...
    _getCurrentStyle : function(el, property, computedStyle) {
        var style, 
            computedStyle = computedStyle || (window.getComputedStyle ? window.getComputedStyle(el, '') : el.currentStyle);
        try{
            style = computedStyle.getPropertyValue(property);
        }catch(e){
            property = property.replace(/^(-)/,"").replace(/-(\S)/g, 
                            function (a, b){ 
    ...
    _tween : function(start, end){
        var style = this.slideBody.style,
            tweenStyle = this._getTweenStyle();
        ...
        tween.onEnd = function(){
            var isEnd = !_this.hasNextPage(),
            	isBegin = !_this.hasPreviousePage(),
            	status = "";
            if(isEnd){
                _this.isLast = true;
                _this.isFirst = false;
