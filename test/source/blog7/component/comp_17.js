	...
	"parseToHTML"	: function (oData){
		...
		if(oData.length == 0){
			...
			Core.Array.foreach(oData, Core.Function.bind2(function (oItem) {
				// 好友删除按钮
				oItem.update = oItem.isblogupdate || oItem.isphotoupdate || oItem.isvblogupdate
						|| oItem.isbbsupdate || oItem.ismusicupdate
						? '<span class="new"><img class="SG_icon SG_icon7" '
							+ 'src="http://simg.sinajs.cn/blog7style/images/common/sg_trans.gif" width="15" '
							+ 'height="15" title="有内容更新，赶快去看一下" align="absmiddle" /></span>'
						: '';
			...
			if($isAdmin == true && this.ismore == 1){
				result += '<div class="moreLink SG_j_linedot1">'
						+ '<span class="SG_more"><a href="http://control.blog.sina.com.cn/blogprofile/profilefriendlist.php" target="_blank">更多</a>&gt;&gt;</span>'
					+ '</div>';
			}else{
