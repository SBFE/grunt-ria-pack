...
$registComp(10001, {
	...
	,"load"			: function(nPage){
		...
		var listUrl = "http://blog.sina.com.cn/s/article_sort_"
							+ scope.$uid + "_" + this.compId + "_" + nPage + ".html";
	...
	,"getArticleLists" : function () {
		...
		for(var i=0; me.currentArticleList[i]; i++) {
			...
			if(newEle) {
				...
				$E('digg_list_'+me.compId+'_'+me.currentArticleList[i]).onclick = (function(tar){
					...
					'ele':eleT
					,'res_id' : me.currentArticleList[i]
					,'res_uid' : scope.$uid
				});
				var eleT = $E('digg_list_'+me.compId+'_'+me.currentArticleList[i]).parentNode.parentNode.getElementsByTagName('b')[0];
		...
		App.getArticlesDetailNumber(scope.$uid, this.currentArticleList
								, Core.Function.bind2(this.renderNumber, this));
...
//        }]
    ...
    ,"getSummaryPics" : function(aid){
        ...
        var $byClz = Core.Dom.getChildrenByClass,
            $nextNode = Core.Dom.nextNode;
