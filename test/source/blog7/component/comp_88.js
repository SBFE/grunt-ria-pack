...
$registComp(88, {
	...
	"load" : function () {
		this.setContent('<div class="cloneWidget" ' + ($isAdmin ? 'style="padding:0px;"' : 'style="padding:0px;"')
			+ '><div class="finance' + this.size + '" ' + (this.size != 210 ? 'style="padding-bottom:15px;"' : '') 
			+ '><div id="comp_' + this.compId + '_content"></div></div>'
			+ ($isAdmin ? '' : '<p class="fincAddToBlog" style="padding-bottom:5px;"><a title="添加到我的博客" href="#" onclick="'
				+ 'Lib.Component.clone(' + this.compId + ', ' + this.size + ');return false;"><img alt="" src="http://'
				+ 'simg.sinajs.cn/blog7style/images/widget/add1.gif" /> 添加到我的博客</a></p>'))
			+ '</div>';
		var comp_size = {
			"210"	: "s",
			"510"	: "m",
				...
				this._parseHq = function (type, code, adnlObj) {
					...
					case "fu":
						...
						var _obj = {
							...
							rate: _change > 0 ? ["+", (_change / sData[7] * 100).toFixed(2), "%"].join("") : [(_change / sData[7] * 100).toFixed(2), "%"].join(""),
