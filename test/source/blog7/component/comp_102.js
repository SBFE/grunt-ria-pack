...
$registComp(102, {
	...
	,tpl: '' +
        '<div class="photoBlog_cell">' +
        	'<div class="photoBlog_view">' +
            '<table>' +
            '<tr>' +
            '<td>' +
            '<a href="{0}" target="_blank"><img class="borderc" src="{5}" alt="{7}" title="{7}"/></a>' +
            '</td>' +
            '</tr>' +
            '</table>' +
            '</div>' +
        	'<p class="photoBlog_nm">{6}<a href="{0}" target="_blank">{1}</a> <span class="count">({2}张)</span></p>' +
            '<p>阅读({3}) 评论({4})</p>' +
        '</div>'
}, 'dymanic');
