...
;(function(){
	...
	var TPL = {
		adCont : [
			'<div id="#{adCon}">',
				'<div id="#{adConBac}" style="z-index:1000; cursor:pointer;">',
					'<div id="#{swfcont}"></div>',
				'</div>',
			'</div>'
		].join('')
	...
	WoMaiAd.prototype = {
		...
		setPos : function(node){
			...
			if($IE6){
				node.style.bottom = document.documentElement.scrollTop +
					parseInt(
						Math.abs(
							(document.documentElement.offsetHeight - h)/2
						),
						10
					);
			}else{
