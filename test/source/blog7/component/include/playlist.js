	...
	,"getNextVideo" : function (fCallBack) {
		...
		if(this.cachelist[this.currentPgNum] == null){
			...
			if(this.isEnd == false){
				...
				Debug.log(((this.currentPgNum - 1)  * this.pageSize + this.currentIndex) + " / " + this.total
					+ "，播放完毕 : " + this.isEnd);
			}else{
				currentVideo = [-1, -1];
				Debug.log("全部播放完毕，展示推荐列表");
			}
			var paginationInfo = this.currentIndex != 1 ? null
								: {  data		: this.cachelist[this.currentPgNum]
									,page		: this.currentPgNum
