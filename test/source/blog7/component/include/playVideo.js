...
//			this.showVideoInfo({});
	...
	,"showVideoInfo" : function (oData) {
		...
		if(lastVideo != null && oData != null){
			...
			if(last != null){
				var html_normal = '<div class="SG_dot"></div><a href="#" '
						+ 'onclick="playMyVideo('+ lastVideo.vid +', '+ lastVideo.uid +');return false;"'
						+ ' title="' + lastVideo.name + '">' + Core.String.encodeHTML(App.format(Core.String.decodeHTML(lastVideo.name), 26)) + '</a>';
