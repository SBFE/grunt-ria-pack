...
$registComp(365, {
	...
	"preLoad": function(y, m){
		...
		this.blog365List = [];
		var 	now = new Date(),
				originYear = now.getFullYear(),
				originMonth = now.getMonth()+1,
				originDate = now.getDate();
				
		var 	currentYear = y || originYear,
				currentMonth = m || originMonth;
			...
			result = ['<div class="cloneWidget">',
				'<div class="cal365zj">',
					'<div class="calBtn">',
						...
						'<a class="btn_next" id="blog365Next" href="javascript:void(0);">▶</a>',
					'</div>',
					'<div class="calCon">',
						'<div class="weekDate">',
							'<span>日</span><span>一</span><span>二</span><span>三</span><span>四</span><span>五</span><span class="nomgr">六</span>',
						'</div>',
						'<ul id="blog365Conn">'].join("")+ result +['</ul>','</div>','</div>', '<div class="cloneLink"><a href="javascript:;" onclick="'
						+ 'Lib.Component.clone(' + this.compId + ', ' + this.size + ');return false;" class="SG_comClone"><img alt="添加到我的博客" src="http://simg.sinajs.cn/blog7style/images/widget/add1.gif">添加到我的博客</a></div></div>'].join("");
		}else{
			result = ['<div class="cal365zj">',
				'<div class="calBtn">',
					'<a class="btn_preview" id="blog365Prev" href="javascript:void(0);">◀</a>',
					'<span class="current" id="blog365curTime">'+currentYear+'年'+currentMonth+'月</span>',
					'<a class="btn_next" id="blog365Next" href="javascript:void(0);">▶</a>',
				...
				'<div class="calCon">',
					'<div class="weekDate">',
						'<span>日</span><span>一</span><span>二</span><span>三</span><span>四</span><span>五</span><span class="nomgr">六</span>',
					'</div>',
					'<ul id="blog365Conn">'].join("")+ result +['</ul>','</div>','</div>'].join("");
		}
	...
	"renderCalendar": function(data){
		...
		for (var item in data){
			...
			var 	child = children[i],
					div = child.getElementsByTagName("div")[0];
