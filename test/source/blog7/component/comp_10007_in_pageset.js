...
$registComp("10007", {
	"pagesize": 5
	,"interfaceSaveSet": new Interface('http://control.blog.sina.com.cn/riaapi/component_config/write_component.php', 'ajax')
	,"interfaceReadSet": new Interface('http://control.blog.sina.com.cn/riaapi/component_config/read_component.php', 'ajax')
	,"interfaceShow" : new Interface('http://control.blog.sina.com.cn/riaapi/appMake_Show.php', 'jsload')
	,"interfaceGetList" : new Interface('http://control.blog.sina.com.cn/riaapi/photoblog/get_abroad_blog_list_by_page.php', 'jsload') //分页获取博文列表
	...
	,"checkUserFlash": function(){
		...
		if (ver < 9) {
			...
			html = '<div class="noFlash_txt">' +
				'您可以在<a href="http://get.adobe.com/flashplayer/" target="_blank">此处下载免费的Adobe flash player installer</a>。成功安装程序后，可返回此处查看flash地图。' +
			'</div>';
	...
	,"getUserSet": function(){
		if (this.isSetOn == null) {
			...
			Core.Function.bind2(function(){
				var usersetHtml = '<div class="vp_login #{cls}"><div>' +
									' 用分类筛选：<select id="comp_' + this.compId + '_type">#{type}</select>' +
									'</div>#{span210start}<a href="#" onclick="Core.Function.bind2(Lib.Component.instances[' + this.compId + '].saveUserSet, Lib.Component.instances[' +
									this.compId +'])();return false;" class="SG_aBtn SG_aBtnB "><cite>确定</cite></a>&nbsp;' +
									'<a href="#" onclick="Core.Dom.removeNode($E(\'comp_' + this.compId + '_set\'));Lib.Component.instances[' +
									this.compId +'].isSetOn=null;return false;" class="SG_aBtn SG_aBtnB "><cite>取消</cite></a>#{span210end}</div>';
				var tplData = {
