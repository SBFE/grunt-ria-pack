...
$registComp(7, {
	...
	,"parse"	: function (oData) {
		...
		var data = [{
			"current"		: ''
			,"sortadd_start": '<a href="http://blog.sina.com.cn/s/articlelist_' + scope.$uid + '_0_1.html">'
			,"name"			: "全部博文"
			,"count"		: oData.total || "0"
			,"sortadd_end"	: "</a>"
		}];
		for(var key in dataHash){
			var oItem = {
				"current"		: (scope.$blogArticleSortSelected == key) ? ' class="current"' : ''
				,"sortadd_start": (scope.$blogArticleSortSelected == key) ? '<strong>'
									: '<a href="http://blog.sina.com.cn/s/articlelist_' + scope.$uid + '_' + key + '_1.html">'
				,"name"			: dataHash[key].name
				,"count"		: dataHash[key].count || "0"
				,"sortadd_end"	: (scope.$blogArticleSortSelected == key) ? '</strong>' : '</a>'
			};
			data.push(oItem);
		}
		...
		i_getRecommend.request({
			...
			,onSuccess	: function (oData){
				if($E("menuList_blog_classList")){
					Core.Dom.addHTML($E("menuList_blog_classList"), '<div class="SG_j_line"></div><ul><li>'
						+ '<div class="menuCell_main">'
						+ '<span class="SG_dot"><a href="http://blog.sina.com.cn/s/articlerecommended_'
						+ scope.$uid + '_1.html">被推荐博文<em>(' + oData.num + ')</em></a></span>'
						+ '</div><div class="menuCell_bot"></div></li></ul>');
				}
			}
			,onError	: function () {}
			,onFail		: function () {}
