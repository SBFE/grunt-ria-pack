	...
	"parseToHTML"	: function (oData){
		...
		if(oData.length == 0){
			...
			Core.Array.foreach(oData, Core.Function.bind2(function (oItem) {
				...
				// 评论删除按钮
				oItem.manage = $isAdmin
					? '<span class="option"><a href="#" onclick="Lib.deleteCommentByAid(\''
							 + oItem.aid + '|' + oItem.comment_id + '\', Core.Function.bind2(Lib.Component.instances['
							+ this.compId + '].reload, Lib.Component.instances[' + this.compId + ']),'+oItem.uid+');return false;" '
						+ 'class="CP_a_fuc">[<cite>删除</cite>]</a></span>'
					: '';
			...
			var result = (this.size == 210
						? '<div class="zComments"><ul class="zCommentsUl">'
						: '<div class="zComments zAvatars"><ul class="zCommentsUl">')
							+ template.evaluateMulti(oData) + '</ul>';
			// 更多链接，只有在博主登录才可以看到，且满足评论大于 10 条
			if($isAdmin && this.more == 1){
				result += '<div class="toDoBar"><span class="SG_more SG_floatR">'
					+ '<a href="http://i.blog.sina.com.cn/blogprofile/profilecommlist.php?type=1" target="_blank">更多</a>&gt;&gt;</span></div>';
			}
	...
	,"setManage"	: function (){
		if($isAdmin && this.getManage()){
			this.getManage().innerHTML  = '<span class="move"><a href="#" '
								+ 'onclick="funcMoveUpDown.up(1);return false;">↑</a><a href="#" '
								+ 'onclick="funcMoveUpDown.down(1);return false;">↓</a></span>'
