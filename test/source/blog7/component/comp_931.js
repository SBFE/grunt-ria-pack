...
$registComp(931, {
	...
	,"parse"	: function (oData) {
		...
		Core.Array.foreach(oData, function(oItem){
			...
			if(data[year] == null){
				data[year] = {
					"currentYear"	: isCurrentYear ? ' fileCell_open' : ''
					,"year"			: year
					,"isshow"		: isCurrentYear ? '' : ' style="display: none;"'
					,"html_data"			: []
				};				
			}
			if(isCurrentMonth){
				data[year].html_data.push('<li class="current"><div class="menuCell_main">'
						+ '<span><strong>' + month + '月(' + oItem.c + ')</strong></span></div><div class="menuCell_bot"></div></li>');				
			}else{
				data[year].html_data.push('<li><div class="menuCell_main">'
						+ '<span><a href="http://blog.sina.com.cn/s/article_archive_'
						+ scope.$uid + '_' + oItem.d + '_1.html">' + month
						+ '月(' + oItem.c + ')</a></span></div><div class="menuCell_bot"></div></li>');	
			}
