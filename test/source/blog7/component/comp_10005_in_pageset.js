	...
	"setManage"	: function () {
		if($isAdmin && this.getManage()){
			this.getManage().innerHTML  = '<span class="move"><a href="#" '
								+ 'onclick="funcMoveUpDown.up(10005);return false;">↑</a><a href="#" '
								+ 'onclick="funcMoveUpDown.down(10005);return false;">↓</a></span>'
	...
	,"getUserSet"	: function () {
		if(this.isSetOn == null){
			...
			var html = ['<div class="vp_login w' + (this.size == 210 ? 4 : (this.size == 510 ? 2 : 1)) + ' borderc">'
						...
						+ '_set\'));Lib.Component.instances['+ this.compId
						+ '].isSetOn=null;return false;" class="SG_aBtn SG_aBtnB "><cite>取消</cite></a>'
						, '</div>'].join("");
			...
			i_getLastestVideo.request({
				GET		: {
					...
					,"pagesize"	: 50
					,"page"		: 1
					,"varname"	: "requestId_cl_" + scope.$uid
				}
				,onSuccess	: Core.Function.bind2(function (oData) {
					...
					if(oData.cateList.length > 0){
						for(var i = 0, len = oData.cateList.length; i < len; i++) {
							...
							option.push('<option value="' + oData.cateList[i].topicid + '" '
									+ (this.tid == oData.cateList[i].topicid ? ' selected="true"' : '')
									+ '>' + cateTitle + '</option>');
						}
					}
