...
//			,"varname"	: "requestId_blog_index_visit_list"
	...
	,"html" : ['<div class="ptCell">'
					, '<p class="pt_img"><a id="linkVisited_#{uid}" href="#{uhref}" target="_blank" '
						...
						, '#{update}#{manage}</p>'
					, '<p class="pt_nm"><a href="#{uhref}"" target="_blank" title="#{name}">#{shortName}</a>#{userV}</p>'
	...
	"parseToHTML"	: function (oData){
		...
		if(oData.length == 0){
			...
			if(	(scope.$pageid == "indexM" || scope.$pageid == "pageSetM")
						&& $isAdmin == true
						&& ((this.size < 730 && this.total > 12) || (this.size == 730 && this.total > 18))){
