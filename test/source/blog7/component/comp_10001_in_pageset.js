		...
		if(this.isSetOn == null){
			...
			App.getArticlesSort(Core.Function.bind2(function (){
				var userset_html = ['<div class="vp_login #{cls} borderc">'
						, '<div>显示方式：<select id="comp_' + this.compId + '_view" #{disable}>#{view}</select>#{tips}'
						+ (this.size != 210 ? '&nbsp;&nbsp;&nbsp;&nbsp;' : '')
						, ' 用分类筛选：<select id="comp_' + this.compId + '_sort">#{type}</select>'
						+ (this.size != 210 ? '&nbsp;&nbsp;&nbsp;&nbsp;' : '<br/>')
						, ' 显示文章篇数：<select id="comp_' + this.compId + '_pagesize">#{pagesize}</select>'
						, '</div>#{span210start}<a href="#" onclick="Core.Function.bind2(Lib.Component.instances[' 
						...
						, '<a href="#" onclick="Core.Dom.removeNode($E(\'comp_' + this.compId
							+ '_set\'));Lib.Component.instances['+ this.compId + '].isSetOn=null;return false;" class="SG_aBtn SG_aBtnB "><cite>取消</cite></a>#{span210end}'
					 	, '</div>'].join("");
				...
				i_getUserSet.request({
					...
					,onSuccess : Core.Function.bind2(function (oData) {
						...
						//准备用于替换模板的数据
						var data = {
							"cls"	: setClass[this.size]
							,"disable"	: this.size == 210 ? 'disabled="true"' : ''
							,"tips"		: this.size == 210 ? '<br/>该模块移至更宽位置方可使用该功能</div><div>' : ''
							,"span210start" : this.size == 210 ? '<span>' :''
							,"span210end" : this.size == 210 ? '</span>' :''
						};
						// 全文还是摘要数据
						data.view = '<option value="0" ' + (this.articleView == 0 ? 'selected="true"' : '')
									+ '>摘要</option><option value="1"' + (this.articleView == 1 ? 'selected="true"' : '')
									+ '>全文</option><option value="2"' + (this.articleView == 2 ? 'selected="true"' : '')
									+ '>标题</option>';
						// 分类数据
						...
						Core.Array.foreach(App.ArticlesSort.data, Core.Function.bind2(function (oItem){
							type.push('<option value="' + oItem.id + '" '+ (this.articleSort == oItem.id ? 'selected="true"' : '')
										+'>' + (this.size == 210 ? Core.String.leftB(oItem.name, 16) : oItem.name) + '</option>');
						}, this));
						data.type = type.join("");
