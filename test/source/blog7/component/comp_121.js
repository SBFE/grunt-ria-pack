	...
	"parseToHTML"	: function (oData,len){		
		...
		if(oData.length == 0){
			...
			var allTips='<div class="morelink"><a href="http://blog.sina.com.cn/s/article_film_'+scope.$uid+'_1.html" target="_blank">查看全部&gt;&gt;</a></div>';
			 ...
			 var innerHTML=['<div class="moviesbox">',
			 				//'<div class="SG_tag">',
			...
			this.getContent().innerHTML = (!$isAdmin ? '<div class="cloneWidget" style="padding:0 0 10px;">' : '')+innerHTML+($isAdmin ? '' : '<div class="cloneLink"><a title="添加到我的博客" href="#" onclick="'
	...
	,"showEmpty" : function () {
		...
		innerHTML=['<div class="moviesbox">',
                   ...
                   // '</ul></div>',					
            		...
            		html,
					'</div>'].join("");
		innerHTML=(!$isAdmin ? '<div class="cloneWidget" style="padding:0 0 10px;">' : '')+innerHTML+($isAdmin ? '' : '<div class="cloneLink"><a title="添加到我的博客" href="#" onclick="'
						+ 'Lib.Component.clone(' + this.compId + ', ' + this.size + ');return false;"><img alt="" src="http://'
						+ 'simg.sinajs.cn/blog7style/images/widget/add1.gif" /> 添加到我的博客</a></div></div>');
		this.setContent(innerHTML);
	}
