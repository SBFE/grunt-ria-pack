...
$registComp(10004, {
	...
	,"insertFlash"	: function () {
		...
		this.getContent().innerHTML = (!$isAdmin ? '<div class="cloneWidget" style="padding:0 0 10px;">' : '') +
					'<div id="' + _cnt + '" class="PrWrap" style="padding: '
					+ this.pad + 'px; width: auto;"></div>'
					+ ($isAdmin ? '' : '<div class="cloneLink"><a title="添加到我的博客" href="#" onclick="'
						+ 'Lib.Component.clone(' + this.compId + ', ' + this.size + ');return false;"><img alt="" src="http://'
						+ 'simg.sinajs.cn/blog7style/images/widget/add1.gif" /> 添加到我的博客</a></div></div>');
		
		// allowScriptAccess always
		var addFlash = Core.Function.bind2(function () {
				var sinaFlash2 = new sinaFlash($_GLOBAL.flashBasicURL + "AlbumV1.swf", "compflash_" + this.compId
					, this.flash_width, this.flash_height, "9", "#FFFFFF", false, "High"
					, "http://www.sina.com.cn/", "http://www.sina.com.cn/", false);
	...
	,"setManage"	: function () {
		if($isAdmin && this.getManage()){
			this.getManage().innerHTML  = '<span class="move"><a href="#" '
								+ 'onclick="funcMoveUpDown.up(10004);return false;">↑</a><a href="#" '
								+ 'onclick="funcMoveUpDown.down(10004);return false;">↓</a></span>'
								+ '<a href="#" onclick="Lib.Component.set(\'' + this.compId
								+ '\');return false;" class="CP_a_fuc">[<cite>设置</cite>]</a>'
	...
	,"getUserSet"	: function () {
		if(this.isSetOn == null){
			...
			var html = ['<div class="vp_login w' + (this.size == 210 ? 4 : (this.size == 510 ? 2 : 1)) + ' borderc">'
						...
						, '</select>'
						, '</div>'
						, '<a href="#" onclick="Core.Function.bind2(Lib.Component.instances[' 
						+ this.compId + '].saveUserSet, Lib.Component.instances[' + this.compId
						+ '])();return false;" class="SG_aBtn SG_aBtnB "><cite>确定</cite></a>&nbsp;'
						, '<a href="#" onclick="Core.Dom.removeNode($E(\'comp_' + this.compId
						+ '_set\'));Lib.Component.instances['+ this.compId + '].isSetOn=null;return false;" class="SG_aBtn SG_aBtnB "><cite>取消</cite></a>'
						, '</div>'].join("");
				...
				,onSuccess	: Core.Function.bind2(function (oData) {
					...
					if(oData.record.length > 0){
						for(var i = 0, len = oData.record.length; i < len; i++) {
							...
							option.push('<option value="' + oData.record[i].ctg_id + '" '
									+ (this.currentCtgid == oData.record[i].ctg_id ? ' selected="true"' : '')
									+ '>' + ctgName + '</option>');
						}
					}
...
window.shareAlbumToTsina=function (option) {
		...
		t = '分享' + (option.owenerNickName||"") + '的图片幻灯：' + (option.title||""),
		u = 'http://sjs.sinajs.cn/blog7swf/AlbumV1.swf?uid=' + scope.$uid+'&ctgid='+Lib.Component.instances[10004].currentCtgid,
		m="http://portrait" + (scope.$uid % 8 + 1) + ".sinaimg.cn/" + scope.$uid + "/blog/180",
		p = ['url=', e(u), '&title=', e(t), '&source=', e(r), '&sourceUrl=', e(l), '&content=', 'utf-8', '&pic=', e(m)].join('');
		function a() {
				window.open([f, p].join(''), 'mb', ['toolbar=0,status=0,resizable=1,width=440,height=430,left=', (s.width - 440) / 2, ',top=', (s.height - 430) / 2].join(''));
		}
