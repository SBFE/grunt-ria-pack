...
$registJob("editor_mjtag", function () {
	...
	/** 热门标签 */
    html = '';
	var hotTags = articleEditorCFG.articleAllTagList.newhot;//.concat(articleEditorCFG.articleAllTagList.tag_Recommend);
//	for (i = 0, len=hotTags.length; i < len; i++) {
//		html += hotTags[i].title ? '<a href="javascript:;" class="ts_txt" e-data="'+ hotTags[i].presetTag +'">' + hotTags[i].title + '</a>'
//            : '<a href="javascript:;" class="ts_tag">' + hotTags[i].presetTag + '</a>';
//	}
    i=0;
    if(hotTags[i]&&hotTags[i].title){
        html += '<a href="javascript:;" class="ts_txt" e-data="'+ hotTags[i].presetTag +'">' + hotTags[i].title + '</a><br/><br/>';
        var tags = hotTags[i].presetTag.split(' ');
        for(i=0, len=tags.length; i<len; i++){
            html += '<a href="javascript:;" class="ts_tag">' + tags[i] + '</a>';
        }
    }
    ...
    __addEvent($E('tagUpDown'), function(){
        updown($E('tagUpDown').className==='down');
    });
    ...
    function updown(isDown){
        var el = $E("tagUpDown");
        el.className = isDown ? 'top' : 'down';
        //el.title = isDown ? '收起' : '展开';
        $E("userTagList").parentNode.style.display = isDown ? '' : 'none';
    }
});
