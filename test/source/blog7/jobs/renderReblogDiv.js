...
$registJob("renderReblogDiv", function(){
    var _addEvent = Core.Events.addEvent,
        ...
        delegateEl, delegateElId,
        reblogDivId = "reblog_"+(+new Date);
    var reblogDivInnerHTML = '<ul>' +
        '<li>' +
            '<a action-type="click_reblog" action-data="{action:\'toQing\'}" href="javascript:void(0);" title="转载到Qing博客">' +
                '<img width="15" height="15" align="absmiddle" src="http://simg.sinajs.cn/blog7style/images/common/sg_trans.gif" class="SG_icon SG_icon205"/>Qing' +
            '</a>' +
        '</li>' +
         '<li>' +
            '<a action-type="click_reblog" action-data="{action:\'toBlog\'}" href="javascript:void(0);" title="转载到博客">' +
                '<img width="18" height="18" align="absmiddle" src="http://simg.sinajs.cn/blog7style/images/common/sg_trans.gif" class="SG_icon SG_icon141"/>博客' +
            '</a>' +
         '</li>' +
         '<li style="display:none;">' +
            '<a action-type="click_reblog" action-data="{action:\'toBlog\'}" href="javascript:void(0);" title="转载原文">' +
                '<img width="18" height="18" align="absmiddle" src="http://simg.sinajs.cn/blog7style/images/common/sg_trans.gif" class="SG_icon SG_icon141"/>转载原文' +
            '</a>' +
         '</li>' +
         '<li style="display:none">' +
            '<a action-type="click_reblog" action-data="{action:\'editor\'}" href="javascript:void(0);" title="采编">' +
                '<img width="18" height="18" align="absmiddle" src="http://simg.sinajs.cn/blog7style/images/common/sg_trans.gif" class="SG_icon SG_icon142"/>采编' +
            '</a>' +
        '</li>' +
         '<li>' +
            '<a action-type="click_reblog" action-data="{action:\'cwb\'}" href="javascript:;" title="发布长微博">' +
                '<img width="18" height="18" align="absmiddle" src="http://simg.sinajs.cn/blog7style/images/common/sg_trans.gif" class="SG_icon SG_icon143"/>长微博' +
            '</a>' +
        '</li>' +
    '</ul>';
...
//    reblogDiv.id = reblogDivId;
    var reblogDiv = SinaEx.createNode('<div id="'+reblogDivId+'" class="turnList" style="display:none;position:absolute;z-index:20;">'
        +reblogDivInnerHTML+'</div>');
    ...
    var onMouseOverReblogBtn = function(e){
        ...
        setTimeout(function(){
			...
			if(!reblogBtn.getAttribute("isreblog")){
				editorState.innerHTML = '<img width="18" height="18" align="absmiddle" src="http://simg.sinajs.cn/blog7style/images/common/sg_trans.gif" class="SG_icon SG_icon142"/>采编';
			}else{
				editorState.innerHTML = '<img width="18" height="18" align="absmiddle" src="http://simg.sinajs.cn/blog7style/images/common/sg_trans.gif" class="SG_icon SG_icon142"/>已采编';
			}
            ...
            setTimeout(function(){
				...
				if(data.srcBlog == "0"){//转载
					reblogDiv.children[0].children[3].style.display = "none";
				}
    ...
    var reblog = function(e){
        ...
        }else if("cwb" === data.action){
            window.open('http://control.blog.sina.com.cn/admin/article/changWeiBo.php?url='+
                        encodeURIComponent("http://blog.sina.com.cn/s/blog_" + blogId + ".html") );
