...
$registJob("sharePhoto", function() {
		var foreach = Core.Array.foreach,
            pageid = scope.$pageid,
            isArt = pageid == "article" || pageid == "articleM",
            uid = scope.$uid,
            nickName,
            //要分享图片 的地址
            nowImgSrc,
...
//        });
		...
		function fnover() {
				...
				shareConStyle.top = Core.Dom.getXY(this)[1] + this.height - minHeight + 4 + "px";
				if (fixAutoHover) {
		...
		if (pageid == "index" || pageid == "indexM") {
				foreach([$E('module_10001'), $E('module_10002'), $E('module_10003')], function(comp) {
