...
UploadImage.prototype = {
    ...
    ,errorLog : function(msg){
        try{
            ...
            var typeEl = createInput('type', 'flash_err_log'),
                pageEl = createInput('page', scope.$PRODUCT_NAME + '_' + scope.$pageid),
                codeEl = createInput('code', 500),
                descEl = createInput('desc', 'getTickets='+Lib.Ticket.requestTickets.join(',')+'||'+msg);
	...
	setParams:function(){
		...
		trace(size);
		$E(this.EMBED_ID+ this.id).setParams(url + '|'+type+'|'+size
	...
	ticketCallback:function(jsonStr){
        ...
        var json = Utils.Json.obj2json(jsonStr);
		ele.addTicket(json);
	},
