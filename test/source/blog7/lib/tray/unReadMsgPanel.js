...
Lib.register('tray.unReadMsgPanel', function(lib){
    var unReadMsgPanel = Core.Class.define(function(ele) {
        ...
        _eventHandle:function(data) {
            ...
            _data.notice  = data.notice + data.like + data.notice_favourite + data.notice_quote + data.invite + data.unreadnotices;//通知(包含喜欢、收藏、转载、关注、好友邀请、好友邀请的处理结果)
            _data.notices = {"notice":data.notice,"like":data.like,"notice_favourite":data.notice_favourite,"notice_quote":data.notice_quote,"invite":data.invite,"unreadnotices":data.unreadnotices};//用于判断输出什么连接
