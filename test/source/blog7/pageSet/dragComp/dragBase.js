        ...
        isInArea: function(pos, area){
            ...
            if ((area.isNoDragIn == false) && area.left <= pos.x && pos.x <= area.right) {
                ...
                if ((area.top <= pos.y) && (pos.y <= (area.top + area.height / 2))) {//上半部分
                    ...
                    if (((area.top + area.height / 2) < pos.y) && (pos.y <= area.bottom)) {//下半部分
                        result.down = true;
                    }
                    else 
                        if (pos.y < area.top) {//在该区域的上面，但不在内部
                            result.above = true;
                        }
                        else {//在该区域的下面，但不在内部
                            result.below = true;
        ...
        resignBorder: function(id, position){
            ...
            if (position == "up") {
                ...
                if (position == "down") {
                    ...
                    if (position == "above") {//说明移动到了某列的上面，但不在列里面
                        var childs = Core.Dom.getChildrenByClass($E(id), "SG_conn");
                        if (childs.length == 0) {
                            $E(id).appendChild(div);
                        }
                        else 
                            if (dragBase.isFobiden(childs[0].id.split('_')[1])) {
                                Core.Dom.insertAfter(div, childs[0]);
                            }
                            else 
                                if (childs[0] == this.whichisdrag) {
                                    this.hiddenBorder();
                                }
                                else {
                        ...
                        if (position == "below") {//说明移动到了某列的下面，但不在列里面
                            var childs = Core.Dom.getChildrenByClass($E(id), "SG_conn");
