		   ...
		   // }
			...
			setTimeout(function(){
				...
				Editor.Utils.ShutOut.prototype.initialize = function(){
					...
					Core.Dom.addHTML(
						$E("mini_tools"),
						'<div id="shut_out'+this.id+'" style="display:none;background:#F1F1F1;position: absolute; z-index: 1025;"></div>'
					);
				};
				...
				onSuccess: function(data){
					...
					if($E('cuscompli_' + data)){			// 说明是编辑原来的文本组件
						...
						if($E('module_' + data)){
							...
							checked = "checked";
							var titlespan = Core.Dom.getChildrenByClass(
								Core.Dom.getChildrenByClass($E('module_' + data),
								'SG_connHead')[0],
								'title'
							)[0];
