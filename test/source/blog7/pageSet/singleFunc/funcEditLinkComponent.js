    ...
    window.editLinkComp = {
        ...
        edit: function(id){
            ...
            if (!window._linkdialog) {
                ...
                Core.Events.addEvent(inputs[0], Core.Function.bind3(_this.inputLength, _this, [inputs[0], /\"|\\|\s|　/, 72]), 'keydown');
                Core.Events.addEvent(inputs[0], Core.Function.bind3(_this.inputfocus, _this, [inputs[0], '输入名称(必填)']), 'focus');
                Core.Events.addEvent(inputs[0], Core.Function.bind3(_this.inputblur, _this, [inputs[0], '输入名称(必填)']), 'blur');
                Core.Events.addEvent(inputs[1], Core.Function.bind3(_this.inputLength, _this, [inputs[1], /\"|\\|\s|　/, 256]), 'keydown');
                Core.Events.addEvent(inputs[2], Core.Function.bind3(_this.inputLength, _this, [inputs[2], /\"|\\|\s|　/, 1000]), 'keydown');
...
.bind2(this)
        ...
        addRow: function(item){
            var _template = '<div><span class="SG_floatR"><span class="SG_floatL"><a class="CP_a_fuc Option1" href="javascript:;" onclick="return false;">[<cite onclick="editLinkComp.editOne(this);">编辑</cite>]</a></span>' +
            '<span class="SG_floatL"><a class="CP_a_fuc Option2" href="javascript:;" onclick="return false;">[<cite onclick="editLinkComp.deleteOne(this);">删除</cite>]</a></span>' +
            '<span class="SG_floatL"><a class="sortUp" href="javascript:;" onclick="editLinkComp.up(this);return false;"><img src="http://simg.sinajs.cn/blog7style/images/common/sg_trans.gif"/></a></span>' +
            '<span class="SG_floatL"><a class="sortDown" href="javascript:;" onclick="editLinkComp.down(this);return false;"><img  src="http://simg.sinajs.cn/blog7style/images/common/sg_trans.gif"/></a><input type="hidden" value="#{otitle}"><input type="hidden" value="#{olink}"><input type="hidden" value="#{odesc}"></span></span>' +
            '<a class="customName" href="#{link}" target="_blank">#{title}</a></div><p class="desc">#{desc}</p>';
...
.bind2(this)                ,
        ...
        getOrignStr: function(id){
            ...
            if (editLinkComp.data != "") {
                for (var i = editLinkComp.data.list.length - 1; i >= 0; i--) {
                    str += '{"title":"' + editLinkComp.data.list[i].title +
                    '","desc":"' +
                    editLinkComp.data.list[i].desc +
                    '","link":"' +
                    editLinkComp.data.list[i].link +
                    '"},';
                }
                if (str.length > 2) {
                    str = str.substring(0, str.length - 1);
