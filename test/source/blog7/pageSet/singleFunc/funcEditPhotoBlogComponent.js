...
(function(){
    var _this = window.editPhotoBlogComp = {
		 ...
		 * 图片博文列表组件 修改
		 */
		,"interfaceModifyComponent": new Interface('http://control.blog.sina.com.cn/riaapi/sys_module/edit_custmod_photoblog_post.php', 'ijax')
		,"interfaceShow" : new Interface('http://control.blog.sina.com.cn/riaapi/appMake_Show.php', 'jsload')
        /**
         * 新增或编辑
         * @param {Object} cid
         */
		...
		,"renderList": function(){
			...
			var template = '<li id="{8}"><div><span class="SG_floatR">' +
								'<span class="SG_floatL"><a href="#" {1} class="CP_a_fuc Option1">[<cite>编辑</cite>]</a></span>' + 
								'<span class="SG_floatL"><a href="#" {2} class="CP_a_fuc Option2">[<cite>删除</cite>]</a></span>' +
								'<span class=" SG_floatL"><a href="#" {3} class="sortUp {5}"><img src="http://simg.sinajs.cn/blog7style/images/common/sg_trans.gif" alt="" /></a></span>' +
								'<span class=" SG_floatL"><a href="#" {4} class="sortDown {6}"><img src="http://simg.sinajs.cn/blog7style/images/common/sg_trans.gif"  alt="" /></a></span>' +
							'</span><a href="{9}" class="customName" target="_blank">{0}</a>{7}</div></li>';
			var htmlStr = '';
		...
		,"showEditor": function(i){
			...
			Utils.limitReg($E('title_input'), /\"|\\|\s|　/); //不允许出现空格
		}
		/**
		 * 关闭条目编辑框
		 */
		 ...
		 */
		,"editorTpl": '' +
			'<li id="title_editor" list_id="{0}" ><div class="editCustomModule">' + 
			'<span class="SG_floatL">' + 
			'<input id="title_input" class="CP_w_fm3" type="text" value="{3}" name=""/>' +
			'</span>' + 
			'<span class="SG_floatL">' + 
			'<a href="javascript:;" {1} class="SG_aBtn SG_aBtnB"><cite>确定</cite></a>' + 
			'</span>' + 
			'<span class="SG_floatL">' + 
			'<a href="javascript:;" {2} class="SG_aBtn SG_aBtnB"><cite>取消</cite></a>' + 
			'</span></div></li>'
		/**
		 * 初始化图片博文对话框
		 */
		,"initDialog": function(){
			var content = '<div class="customModuleListPop">' +
				            '<div class="addCustomModule addCustomModule_ptb">' +
							'<span class="SG_floatL"><input id="inputName" class="CP_w_fm2" type="text" value="输入名称(必填)" name="" /> <em class="SG_clewtxta">* 必填</em></span>' + 
							'<span class="SG_floatR">分类： <select id="selectCate"><option value="">加载中</option></select>&nbsp;&nbsp;&nbsp;&nbsp;呈现方式： <select id="selectStyle"><option value="0">图片</option><option value="1">文字</option></select></span>' +
				            '<div class="clearit"></div>' +
				            '<div class="customPhotoBlog" style="padding-bottom:10px">' +
				            	'<div class="customPhotoBlog_list SG_j_linedot1">' +
				                    '<ul id="pbListUl">' +
				                    '</ul>' +
				                '</div>' +
				                '<div id="pbListPage"  class="SG_page">' +
					                '<ul class="SG_pages">' +
					                '</ul>' +
				            	'</div>' +
				            '<div class="clearit"></div>' +
			...
			Utils.limitReg($E('inputName'), /\"|\\|\s|　/); //不允许出现空格
