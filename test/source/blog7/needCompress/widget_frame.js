...
_ZWF.initWidget = function(embarg,exe){
	...
	if(m && m[1]){
		...
		if(blog)
			_COMM.loadXMLDoc(_ZWF.hubServer+"?"+embarg+"&l="+_COMM.auth.bLogin,_ZWF.gotWidgetInfo);
		else
...
_ZWF.loadStaticWidget = function(otmp,surl){
	...
	for(var i=0;i<embs.length;i++){
		...
		ts += ' onload="if(this.src.search(\'empty.html\')!=-1){this.src=unescape(\''
			+escape(fs)+'\')}" src="'+_ZWF.staticServer
			+'empty.html" style="position:relative;cursor:default;"  scrolling="no" allowTransparency="true"  width="'+w+'px" height="'+h+'px"  border="0" frameborder="0" ></iframe>';
		el.innerHTML = ts; 
		var oif = _ZWF.$(fid);
		if(!otmp.embs)
			otmp.embs = [];
		...
		if( opv.uid ){
			...
			if(typeof(opv.nick)!="string" || opv.nick=="" || opv.nick=="null" ){
				...
				if(udat && udat[osina.uid])
					osina.nick = udat[osina.uid];
				else if( _ZWF.sinaUsers[osina.uid] && typeof(_ZWF.sinaUsers[osina.uid])=="string" )
					osina.nick = udat[osina.uid];
				else
					_ZWF.importJS("http://uic.sinajs.cn/uic?type=nick&uids="+osina.uid+"&varname=_tempSianUidDate_"+osina.uid,"reqSinaUidData"+osina.uid,function(){ eval("_ZWF.gotSinaUidData('"+wgtid+"','"+osina.uid+"')")});
			}
		}
...
_ZWF.initEmb = function(wgtid){
	...
	if(!itms){
		if(owgt.exe && owgt.exe.funName ){
			...
			_ZWF.importJS(fn,null,function(){eval('_ZWA.'+owgt.exe.funName+'("'+owgt.exe.argStr.replace(/\"/g,'\\"')+'")')});
			return true;
		}else{
			return null;
		}
...
_ZWF.fwin.show = function(id){
	...
	if(ofw.bAutoHide){
		...
		document.body.onmousedown = function(){eval('_ZWF.fwin._hide("'+id+'")')};
...
_ZWF.utl.showNotify = function(oarg){
	...
	var ts = '<div style="width:100%;height:100%;'+_PNG.getBgStr(_ZWF.staticServer+"/utility/notify/images/notify_bg_1.png")+'">'
	+'<div onclick="_ZWF.utl.hideNotify();" style="float:right;cursor:pointer;position:relative;width:28px;height:24px;"></div>'
	+'<div onclick="_UTL.open(\''+oarg.link+'\')" style="position:relative;width:272px;height:64px;padding-top:16px;cursor:pointer;filter:progid:DXImageTransform.Microsoft.Alpha(style=0,opacity=100);">'
...
_ZWF.utl.fade = function(divID,bIn,step,beg){
	...
	if(typeof(odiv.style.filter) != "undefined")
		odiv.style.filter = bf ? "progid:DXImageTransform.Microsoft.Alpha(opacity="+beg+")" : "none";
	else
		odiv.style.opacity = beg/100;
	if(bf)
		_ZWF.utl.fadeTimer = window.setTimeout('_ZWF.utl.fade("'+divID+'",'+bIn+','+step+','+beg+')',100);
	else if(_ZWF.utl.onFadeEnd)
		try{
